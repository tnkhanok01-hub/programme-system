...
export default function AdminHomepage() {
  ...
  const handleEdit = (prog: Programme) => {
    setEditForm({
      ...prog,
      start_date: prog.start_date?.slice(0, 10),
      end_date: prog.end_date?.slice(0, 10),
    })
  ...
  const handleUpdate = async () => {
    ...
    if (res.ok) {
      setProgrammes(prev => prev.map(p => p.id === editForm.id ? data.programme : p))
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Delete this programme? This action cannot be undone.')) return
    const token = await getToken()
    if (!token) return
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      case 'Approved': return { bg: 'rgba(16,185,129,0.12)', color: '#10b981', dot: '#10b981', icon: CheckCircle }
      case 'Rejected': return { bg: 'rgba(239,68,68,0.12)', color: '#ef4444', dot: '#ef4444', icon: XCircle }
      case 'Pending': return { bg: 'rgba(245,158,11,0.12)', color: '#f59e0b', dot: '#f59e0b', icon: AlertCircle }
      default: return { bg: 'rgba(148,163,184,0.12)', color: '#94a3b8', dot: '#94a3b8', icon: Clock }
    }
  }
    ...
    <div style={{ minHeight: '100vh', background: '#080f1a', display: 'flex', fontFamily: "'Inter', -apple-system, sans-serif", color: '#e2e8f0' }}>
      ...
      <aside style={{
        ...
        <nav style={{ padding: '14px 10px', flex: 1 }}>
          ...
          {navItems.map(item => {
            ...
            return (
              <button key={item.id} onClick={() => setActiveNav(item.id)} style={{
                ...
                marginBottom: '2px', textAlign: 'left', transition: 'all 0.12s',
              }}>
                <Icon size={15} />
                {item.label}
          ...
          <div style={{ marginTop: '20px', paddingTop: '14px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <button onClick={() => router.push('/create-programme-form')} style={{
              width: '100%', display: 'flex', alignItems: 'center', gap: '9px',
              padding: '9px 10px', borderRadius: '7px', border: 'none', cursor: 'pointer',
        ...
        <div style={{ padding: '14px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '9px', padding: '9px', borderRadius: '9px', background: 'rgba(255,255,255,0.03)' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ margin: 0, fontSize: '12px', fontWeight: 500, color: '#e2e8f0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {profile?.full_name || 'Admin'}
              </p>
              <p style={{ margin: 0, fontSize: '10px', color: '#6b7280' }}>Administrator</p>
            </div>
            <button onClick={handleLogout} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#6b7280', padding: '3px', borderRadius: '4px' }}>
              <LogOut size={13} />
            </button>
          </div>
        </div>
      ...
      <main style={{ flex: 1, marginLeft: '220px', padding: '28px 32px', overflowX: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '28px' }}>
          <div>
            <h1 style={{ fontSize: '20px', fontWeight: 700, margin: 0, letterSpacing: '-0.02em', color: '#f8fafc' }}>
              Admin Dashboard
            </h1>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <button style={{
              ...
              borderRadius: '9px', width: '38px', height: '38px', display: 'flex',
              alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#6b7280',
            }}>
              <Bell size={15} />
              {stats.pending > 0 && (
                <span style={{ position: 'absolute', top: '7px', right: '7px', width: '7px', height: '7px', borderRadius: '50%', background: '#f59e0b', border: '1.5px solid #080f1a' }} />
              )}
            </button>
            <button onClick={() => router.push('/create-programme-form')} style={{
              ...
              padding: '9px 16px', color: 'white', fontSize: '13px',
            ...
            return (
              <div key={i} style={{ background: '#0c1526', border: `1px solid ${card.border}`, borderRadius: '12px', padding: '16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
                  <div style={{ width: '34px', height: '34px', borderRadius: '9px', background: card.bg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ margin: '2px 0 0', fontSize: '22px', fontWeight: 700, color: '#a78bfa', letterSpacing: '-0.03em' }}>
              RM {stats.totalBudget.toLocaleString('en-MY', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </p>
          </div>
          <div style={{ marginLeft: 'auto', display: 'flex', gap: '24px' }}>
            {[
              { label: 'Avg per programme', value: stats.total > 0 ? `RM ${(stats.totalBudget / stats.total).toFixed(0)}` : 'RM 0' },
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    borderRadius: '7px', padding: '7px 10px 7px 30px',
                    color: '#e2e8f0', fontSize: '12px', outline: 'none', width: '180px',
                  }}
                />
              </div>
              {/* Status filter */}
              <div style={{ display: 'flex', gap: '4px' }}>
                {statuses.map(s => (
                  <button key={s} onClick={() => setFilterStatus(s)} style={{
                    ...
                    {s}
                    {s !== 'All' && <span style={{ marginLeft: '4px', fontSize: '10px', opacity: 0.7 }}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  {['Programme', 'Category', 'Dates', 'Venue', 'Budget', 'Status', 'Actions'].map(h => (
                    <th key={h} style={{ padding: '11px 14px', textAlign: 'left', color: '#374151', fontWeight: 500, fontSize: '11px', letterSpacing: '0.04em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={7} style={{ padding: '48px', textAlign: 'center', color: '#374151' }}>
                      <BookOpen size={24} style={{ marginBottom: '8px', opacity: 0.4 }} />
                      <p style={{ margin: 0, fontSize: '13px' }}>No programmes found</p>
                    </td>
                  </tr>
                ) : filtered.map((prog, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: '12px 14px', maxWidth: '200px' }}>
                        <p style={{ margin: 0, fontWeight: 500, color: '#f1f5f9', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{prog.name}</p>
                      </td>
                      {/* Category */}
                      <td style={{ padding: '12px 14px' }}>
                        <span style={{ fontSize: '11px', padding: '3px 8px', borderRadius: '5px', background: 'rgba(167,139,250,0.1)', color: '#a78bfa', fontWeight: 500 }}>
                          {prog.category || '—'}
                        </span>
                      </td>
                      {/* Dates */}
                      ...
                      <td style={{ padding: '12px 14px', color: '#6b7280', maxWidth: '140px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '12px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          <MapPin size={11} />
                          {prog.venue || '—'}
                        </div>
                      </td>
                      {/* Budget */}
                      <td style={{ padding: '12px 14px', color: '#e2e8f0', whiteSpace: 'nowrap', fontVariantNumeric: 'tabular-nums' }}>
                      ...
                      <td style={{ padding: '12px 14px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                          {prog.status === 'Pending' && (
                            <>
                              <button onClick={() => handleApprove(prog.id)} title="Approve" style={{
                                background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.2)',
                                borderRadius: '6px', padding: '5px 8px', cursor: 'pointer', color: '#10b981',
                                display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', fontWeight: 500,
                              }}>
                                <CheckCircle size={12} />Approve
                              </button>
                              <button onClick={() => handleReject(prog.id)} title="Reject" style={{
                                background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.15)',
                                borderRadius: '6px', padding: '5px 8px', cursor: 'pointer', color: '#ef4444',
                                display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', fontWeight: 500,
                              }}>
                          ...
                          <button onClick={() => handleEdit(prog)} style={{
                            background: 'rgba(167,139,250,0.1)', border: '1px solid rgba(167,139,250,0.15)',
                            borderRadius: '6px', padding: '5px 7px', cursor: 'pointer', color: '#a78bfa',
                          }}>
                            <Pencil size={13} />
                          </button>
                          <button onClick={() => handleDelete(prog.id)} style={{
      ...
      {showEditModal && (
        ...
        >
          <div style={{
            ...
            borderRadius: '14px', padding: '28px', width: '100%', maxWidth: '540px',
            margin: '16px',
          }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
              ...
              <div>
                ...
                <select
                  ...
                  style={{
                    width: '100%', padding: '9px 11px', borderRadius: '7px',
                    background: '#0c1526', border: '1px solid rgba(255,255,255,0.08)',
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '22px' }}>
              ...
              <button onClick={handleUpdate} disabled={actionLoading} style={{
