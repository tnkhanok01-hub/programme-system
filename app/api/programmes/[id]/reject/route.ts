...
export async function POST(
  ...
  if (callerRole !== 'admin' && callerRole !== 'superadmin') {
    return NextResponse.json({ error: 'Forbidden. Only admins can reject programmes.' }, { status: 403 });
  }
  ...
  if (programme.status === 'Rejected') return NextResponse.json({ error: 'Programme is already rejected.' }, { status: 409 });
