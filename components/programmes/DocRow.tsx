...
export default function DocRow({ doc, phaseInfo, canUpload, onPreview, onDownload, onDelete }: {
  ...
  const { t } = useTheme()
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 12px', background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: '8px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '9px', minWidth: 0 }}>
        ...
        <div style={{ minWidth: 0 }}>
          <p style={{ margin: 0, fontSize: '12px', color: t.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '180px' }}>{doc.file_name}</p>
