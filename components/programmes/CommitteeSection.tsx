...
export default function CommitteeSection({
  ...
  const handleApprove = async (memberId: string) => {
    try {
      const res = await fetch(`/api/programmes/${programmeId}/committee`, {
        ...
        body: JSON.stringify({ programme_id: programmeId, member_id: memberId, action: 'approve' }),
      })
      const data = await res.json()
  ...
  const handleReject = async (memberId: string) => {
    try {
      const res = await fetch(`/api/programmes/${programmeId}/committee`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ programme_id: programmeId, member_id: memberId, action: 'reject' }),
      })
      const data = await res.json()
  ...
  const handleAddAll = async () => {
    ...
    try {
      ...
      if (addedMatrics.size > 0) {
        ...
        if (selfEntry) await handleApprove(selfEntry.id)
      }
    } catch (err: any) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {isApproved && !canManage && (
          isMember ? (
            ...
            <button onClick={() => {
              setJoinError('')
              document.getElementById('join-panel-' + programmeId)?.scrollIntoView({ behavior: 'smooth', block: 'nearest' })
            }} style={{
              display: 'flex', alignItems: 'center', gap: '5px',
        ...
        {canManage && (
          <button onClick={() => { setShowAddPanel(v => !v); setAddResults([]); setPendingMembers([]); setSearchQuery('') }}
            style={{
              display: 'flex', alignItems: 'center', gap: '5px',
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '4px' }}>
                <button onClick={handleAddAll} disabled={addLoading}
                  style={{
                    ...
                    background: addLoading ? 'rgba(99,102,241,0.4)' : 'linear-gradient(135deg, #4f46e5, #6366f1)',
      ...
      {/* ── Join panel (non-members on approved programmes) ── */}
          ...
          <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
            ...
            <button onClick={handleJoin} disabled={joining}
              style={{
                display: 'flex', alignItems: 'center', gap: '6px',
                padding: '9px 18px', borderRadius: '7px', border: 'none',
                background: joining ? 'rgba(99,102,241,0.4)' : 'linear-gradient(135deg, #4f46e5, #6366f1)',
                color: 'white', fontSize: '13px', fontWeight: 600,
                cursor: joining ? 'not-allowed' : 'pointer',
                fontFamily: 'inherit', whiteSpace: 'nowrap', flexShrink: 0,
              }}>
              {joining
                ? <><RefreshCw size={13} style={{ animation: 'spin 0.8s linear infinite' }} />Joining...</>
                : <><UserPlus size={13} />Confirm Join</>}
            </button>
          </div>
          {joinError && (
            <div style={{
